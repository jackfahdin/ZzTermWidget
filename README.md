...
- 支持花样下划线（SGR 4:1-4:5 五种样式：单线/双线/波浪/虚线/点线，4:0 关闭，58/59 独立下划线色设置与复位）。
- 支持 SGR 38/48 冒口子参数真彩（38:5:n、38:2::r:g:b 与 48 同款，容忍色彩空间空位）。
- 支持编程连字渲染（可选开关，默认关）：开启时等宽连字字体下 ASCII 运算符序列渲染为连字字形（字体默认整形），关闭时显式禁用 liga/calt 特性（Qt 基线 6.8）；含样式边界不跨片段、跨度脏区配套扩展；`TerminalDisplay::setLigaturesEnabled(true)` 开启。
